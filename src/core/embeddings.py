...
def load_model(model_name: str = MODEL_NAME, *, half: bool = False):
    ...
    По умолчанию — полная точность, та самая, в которой модель обучалась.
    Половинная (half=True) втрое быстрее на видеокарте: 348 текстов/с против 103,
    полный корпус 47 минут вместо 158. Замерено, что счёт она не портит — на 4096
    векторах ни одного «не числа», косинус с полной точностью не ниже 0,9995,
    из десяти ближайших соседей совпадает в среднем 9,95.

    Умолчанием она всё же не стала. Эти векторы считаются один раз, и на них
    стоит всё остальное: группировка, признаки, отбор топа, проверка на прошлом.
    Полтора часа разницы платятся однажды, а оговорка «мы считали в пониженной
    точности» осталась бы в методологии навсегда и её пришлось бы защищать.
    Флаг нужен для быстрых прогонов на подвыборке, когда важна не сама выдача,
    а то, проходит ли конвейер целиком.
    """
    try:
        import torch
